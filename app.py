...
def main():
    ...
    openai_api_key = os.getenv("openai_api_key")  # Note: not used here, but included if you need it

    if st.session_state.get_weather:
        with st.spinner("🌤️ Gathering weather magic..."):
            weather_data = get_weather_data(city, weather_api_key)
            
            # Display the raw weather data for debugging
                ...
                if main_data and wind_data and weather_description:
                    ...
                    with col1:
                        ...
                        if temp is not None:
                            ...
                            st.metric("Temperature 🌡️", f"{temp_celsius:.1f}°C", help="Current temperature")
                        else:
                            st.error("Temperature data is missing.")
                    
                    with col2:
                        humidity = main_data.get('humidity', 'N/A')
                        st.metric("Humidity 💧", f"{humidity}%", help="Relative humidity")
                    
                    with col3:
                        wind_speed = wind_data.get('speed', 'N/A')
                        st.metric("Wind Speed 🌬️", f"{wind_speed} m/s", help="Wind speed")
